...
function validateField(field) {
    ...
    if (field.name === "email" && field.validity.typeMismatch) {
        field.setCustomValidity("Введите корректный e-mail, например example@mail.ru");
    }
    if (field.name === "phone" && field.validity.patternMismatch) {
        field.setCustomValidity("Введите корректный номер телефона, например +79991234567");
    }
...
contactForm.addEventListener("submit", (e) => {
    ...
    if (!contactForm.checkValidity()) {
        ...
        if (email?.validity.typeMismatch) {
            email.setCustomValidity("Введите корректный e-mail, например example@mail.ru");
        }
        if (phone?.validity.patternMismatch) {
            phone.setCustomValidity("Введите корректный номер телефона, например +79991234567");
        }
